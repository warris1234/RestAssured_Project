package RestAssuredCRUDMethodTesting;

import io.restassured.http.ContentType;
import io.restassured.response.Response;
import io.restassured.specification.RequestSpecification;
import org.json.simple.JSONObject;
import org.testng.Assert;
import org.testng.annotations.Test;
import static io.restassured.RestAssured.*;

public class PutRequest
{
    @Test
    public void putUpdation()
    {
        JSONObject jo = new JSONObject();
        jo.put("lastName","Shaik");
        jo.put("age","27");
        jo.put("CompanyName","Cognizant");
        jo.put("ClinetName","GSK");
        jo.put("Role","Associate");
        jo.put("InHandSalary","79010");
        // Build request
     RequestSpecification req=  given();
     req.contentType(ContentType.JSON);
     req.body(jo.toString());
   Response response =  req.put("http://localhost:3000/users/20");
   response.then().log().all();
         int code =response.getStatusCode();
      Assert.assertTrue(code == 200|| code == 201,"Unexpected code:" +code);
     String statusLine =   response.getStatusLine();
        Assert.assertTrue(
                statusLine.equals("HTTP/1.1 200 OK") || statusLine.equals("HTTP/1.1 201 Created "),
                "Unexpected Status Line: " + statusLine);
    }
}
